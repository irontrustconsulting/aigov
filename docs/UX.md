...
- **Intake / registration wizard** (`UI-F1-INTAKE`, composition-debt cleared UI-C2) — adoption face dominant, as designed. `UX-1` (capture-once) and `UX-2`/`UX-4` (opinionated default, shown reasoning) bite hardest: one pass — drill-down or "not in catalogue" → structured capture → prefill confirm → use-case create — silently triggers classification; a gate (the context-question sub-surface, the reviewer hand-off) surfaces only when `requires_context` or the answer genuinely needs the owner's decision. Prefilled from the catalogue only (`DF1-4`: the bridge/prior-assessment/cross-tenant rungs of the §3 ladder are not yet wired); every prefilled fact shows its basis and source, override is presentational only pending a write-back path (`DF1-8`) [Clarified by CAT-4: value not written back; disposition recorded per INV-83]. Tier + basis are reasoning-first (`UX-4`) at the resolved-tier and context-outcome screens. Role-aware (`UX-5`): system_owner drives the whole spine; a bare contributor/member meets an empty-state, not a dead control, since capture is system_owner-only; reviewer/authoriser/auditor get an assurance-read message — the wizard has nothing for them to act on. "Whose-court" closes the surface, reading the lifecycle gate vector rather than a dedicated approval-status field. *UI-C2 composition pass:* drill-down is an in-place single-open accordion (D-56) — no page transitions between category/sub-category/vendor/product; the user is never navigated away from the drill-down panel while exploring. Vendor and product rows carry branded `LogoTile`; in-house exit always visible. Structured-capture step uses the FE-4 input family (`UX-2` select-first, `UX-3` opinionated defaults). Prefill confirm uses `PrefillWithBasis` (§1.5 spine, FE-5) with value + basis + provenance on one card per fact. All steps are now under `PageScaffold`/`PageHeader` (INV-69/INV-70 compliant).
...
- **Clearances** (`UI-F10-CLEARANCE`) — assurance face for the act, but the read is deliberately face-neutral: `UX-5` (SoD-visible) bites hardest — every governance-role caller can read the vendor-grouped clearance status list (it's an inspectable status, not a worklist; copy never assumes the reader is the authoriser), while the set-clearance control itself is present only for `authoriser` and absent, not disabled, for every other role. A product row is disabled-with-reason (not absent) until its parent vendor clears, since that's a transient lifecycle-order block, not an SoD bar. Prefilled from nothing — this is a status read plus a small act, not a capture surface. "Whose-court" is closed from two directions: the F2 dashboard's your-court and the system drill-in both deep-link an authoriser-court row here when (and only when) the block is the vendor/product clearance gate specifically, not any authoriser-attributed block (a deployment-authorisation block keeps its existing `AuthorisePanel` destination). Born INV-69/INV-70 compliant.

...
**Current through:** DM-S4b (intended-use category, envelope triage, gate-1 governing-subcategory refinement — `use_case.purpose` column **dropped**; `use_case.product_category_id uuid NULL FK(product_category)` + index added (DM-S4b migration `d5c81f9fd50d`); `classification.off_label boolean NOT NULL DEFAULT false` added; `ClassificationDisposition` Python enum (`AUTHORITATIVE`/`DOWN_SELECTION`) added; `resolve_classification` is now use-case-aware: when `use_case.product_category_id` is set, governs from that category's primary mapping — INV-82, D-71; three-way registration branch: `REQUIRES_CONTEXT` → gate-2 seam snapshot with `off_label=True` when product present but no declared category (D-72); `AUTHORITATIVE` → `APPROVED` + stamps `eu_tier` immediately; `DOWN_SELECTION` → `PENDING_REVIEW`, `eu_tier` NOT stamped (reviewer sign-off stamps it, D-73); `ClassificationRead` gains `status` (WI-6b, D-73); wizard `REGISTERED` branch adds `status=pending_review` → whose-court arm; use-case step replaces free-text purpose with membership-constrained `SingleSelect` + "Other / not listed" sentinel (FE-31); new `GET /v1/catalogue/products/{id}/categories` endpoint; INV-82 minted; D-71, D-72, D-73 minted; FE-31 minted. **1 migration, 2 schema columns added (`use_case.product_category_id`, `classification.off_label`), 1 schema column dropped (`use_case.purpose`), 1 route added (`GET /catalogue/products/{id}/categories`).** Prior: DM-S4a (system intake prefill seeding — `SEED_INTAKE` reducer seeds `operatorRoleId`/`hostingModelId`/`lifecycleStage`/`purpose` from `FieldPrefills` response (empty-guarded; draft-restored values always take precedence); `IntakeCaptureStep` converted from local `useState` to fully wizard-state-controlled (values as props, changes via `onFieldChange`); basis captions (FE-30) rendered below seeded selects — "Catalogue curated — confirm or update" / "Derived — confirm or update"; `SingleSelect` display-equals-submit invariant (INV-81) — always renders `<option value="" disabled hidden>` first so `value=""` never silently selects option-1; two new nullable columns `hosting_model_id` (UUID FK → `hosting_model.id` ON DELETE SET NULL) + `intended_use` (Text) added to `catalogue_product`; `FieldPrefill`/`FieldPrefills` added to `PrefillResponse` (additive — `PrefillStep` reads `facts` only, unaffected); `_build_field_prefills` in `prefill_service.py` (deployer looked up by `code="deployer"`, `lifecycle_stage=PRODUCTION` derived, catalogue columns when non-null); `seed_catalogue_prefill.py` idempotent curation seed (61 products curated); D-68, D-69, D-70 minted; INV-81 minted; FE-30 minted; FE-4 ALTER (SingleSelect placeholder). **1 migration, 2 schema columns (`catalogue_product.hosting_model_id`, `catalogue_product.intended_use`), 0 route delta.** Prior: DM-S3b (dashboard `DraftResumeIndicator` (FE-29) — kit component `packages/ui/src/kit/draft-resume-indicator.tsx`, navigation-only banner on `PortfolioHub` for `system_owner` with an active draft; wired to `useActiveDraft({ enabled: roleKeys.has("system_owner") })`; rendered as first child of `<PageScaffold>` in both non-admin settled returns, excluded from loading/error returns (DF-S3b-1); descriptor resolves `catalogueProductName` → `name` → "Untitled registration" (DF-S3b-2). INV-80 minted and enforced by `tests/test_systems.py::TestDeleteRoutesAbsent`. D-67 authored; **Branch B executed** (1 bare system on tenant `6be51846-1261-489b-b55f-cb4d6b2e3fcc` — `useSystems()`, `zeroUseCaseSystems`, A2 nudge retained unchanged; retirement deferred, DF-S3b-3); `useActiveDraft()` gains optional `enabled` param. DF-S3b-1..3 minted. **0 migrations, 0 schema delta, 0 route delta this sprint.** Prior: DM-S3 (draft staging & resume — `draft_registration` table (TENANT RLS, one active per user via plain unique constraint, owner-filtered access, CASCADE FKs — INV-79, D-66); four draft endpoints `POST/GET/PATCH/DELETE /v1/draft-registrations`; `POST /v1/registrations` gains optional `draft_id` for atomic discard (SV-3); wizard persist-on-transition and front-door resume via `ResumePrompt` (FE-28); DF-D3-1..6 minted). Prior: DM-S2 (atomic use-bound registration — `POST /v1/systems` removed (INV-78); `POST /v1/registrations` atomic system + use-case + classification in one transaction (D-65); `GET /v1/catalogue/products/{id}/prefill` added for by-product prefill before system exists (DF-D2-2); four use-distinguishing context controls relocated from intake-capture step to use-case step (DF-D1-2 closed); wizard reducer gains `INTAKE_DONE`/`REGISTERED`; `useRegister` replaces `useCreateSystem`+`useCreateUseCase`; `SystemCreate` contract retired; DF-D2-1, DF-D2-3 minted). Prior: DM-S1 (context relocation — `usage_context_id`, `human_oversight_type_id`, `data_category_ids`, `affected_party_ids` moved from `system`/`system_data_category`/`system_affected_party` to `use_case`/`use_case_data_category`/`use_case_affected_party`; DPIA/FRIA feeder pre-fill and DPIA applicability now per-use-case; D-63, D-64, INV-76, INV-77, DF-D1-1..4 minted). Earlier: UI-V2-DEPTH-LAYOUT (elevation + surface-layering tokens FE-26; `SectionGroup` + `PageScaffold` width prop FE-27; `VerdictChip` label map FE-16 ALTER; dashboard zero-systems scaffolded-empty D-61; INV-74, INV-75, DF-V2-1..4 minted; `UI-V2-OPERATOR-SKIN` formally retired — absorbed into C-track). Sprints 1–7b + UI-F1..F8 + UI-F9 + UI-V0 + UI-V1 + UI-C0 + UI-C1 + UI-C2 + UI-V2 + DM-S1 + DM-S2 built. Per-surface composition passes (F1–F8) tracked in the composition-debt register below.
...
**DM-S2 (INV-78):** `POST /v1/systems` **removed**. Registration is now atomic via `POST /v1/registrations` (see Atomic registration below). The remaining system routes are: `GET /v1/systems` (any member), `GET /v1/systems/{id}` → `SystemDetail` (resolved labels, `use_case_count`, per-use-case states), `PATCH /v1/systems/{id}` (updates a subset; changing `catalogue_product_id` with use cases present → 409). Gates: `PATCH` `system_owner`; `GET` any member. Audit `system.created`/`updated`.

### Atomic registration (`POST /v1/registrations`)
`POST /v1/registrations` (DM-S2, D-65) — single atomic endpoint, single transaction: creates `System` (deployment-stable facts: `name`, `is_custom`, `catalogue_product_id`, `operator_role_id`, `hosting_model_id`, `lifecycle_stage`, `owner_user_id`, `purpose`) + first `UseCase` (use-distinguishing context: `title`, `intended_use_category_id` (FK `product_category`, nullable — **DM-S4b:** replaces `use_case_purpose`), `usage_context_id`, `human_oversight_type_id`, `data_category_ids`, `affected_party_ids`) + snapshots `Classification` via a three-way branch on `ClassificationDisposition` (**DM-S4b,** D-71, D-73, INV-82): `REQUIRES_CONTEXT` → gate-2 seam snapshot (`off_label=True` when product present but no declared category, D-72); `AUTHORITATIVE` → `APPROVED` snapshot + stamps `eu_tier` immediately; `DOWN_SELECTION` → `PENDING_REVIEW` snapshot, `eu_tier` NOT stamped (reviewer sign-off via `POST .../classification/sign-off` stamps it, D-73) — all via `db.flush()` within the same request transaction (INV-27). Returns `RegistrationRead { system: SystemDetail, use_case: UseCaseRead, classification: ClassificationRead }`. **DM-S4b:** `ClassificationRead` now carries `status` (WI-6b); wizard branches on `status=pending_review` → whose-court without a separate lifecycle call (D-73). Gate: `system_owner`. INV-78: the only route that constructs a `System`. Rollback on any mid-handler exception leaves no orphan rows (verified by `TestRegistrationAtomicity`). `db.flush()` is used inside the handler; caller's `get_tenant_db` commits at request end.

### Catalogue prefill
`GET /v1/systems/{id}/prefill` → `PrefillResponse` (`catalogue_product_id`, `facts` — `key`, `value`, `source_url`, `source_label`, `last_checked_at`, `provenance`) — display-only confirm/amend panel. No product (incl. `is_custom`) → `200` empty, never `404`. No write-back. **DM-S2 (DF-D2-2):** `GET /v1/catalogue/products/{product_id}/prefill` added — same `PrefillResponse` shape, keyed by `catalogue_product_id` before a system exists. `prefill_service.get_prefill_by_product(product_id, db)` is the shared resolver; `get_prefill(system_id, db)` delegates to it after resolving the system's product. Gate: any tenant member (`get_tenant_db`). No audit (pure read).

### Use-case registration
**DM-S2:** Use-case creation is now only via `POST /v1/registrations` (atomic — see above). `POST /v1/use-cases` is retained for standalone use-case creation (not from the wizard). **DM-S1 (D-63):** `UseCaseCreate` carries use-distinguishing context: `usage_context_id`, `human_oversight_type_id` (FK vocs, `ON DELETE SET NULL`), `data_category_ids`/`affected_party_ids` (persisted to `use_case_data_category`/`use_case_affected_party` — TENANT RLS, `tenant_id`-partitioned). `UseCaseRead` returns all four resolved (`usage_context`/`human_oversight_type` as `VocabItemOut`, `data_categories`/`affected_parties` as lists). DPIA applicability and FRIA/DPIA feeder pre-fill now read from `use_case_data_category`/`use_case_affected_party`. **DM-S4b:** `use_case_purpose` (free-text field) **dropped** from `UseCaseCreate`/`UseCaseRead`; `intended_use_category_id uuid | null` added — the declared membership category for this use case (FE-31, D-71). The register-fact AIIA snapshot no longer includes a `use_case.purpose` source (assessment_service.py, DM-S4b).

### Classification — bridge (gate 1)
Auto-resolved on use-case creation via the reference bridge (`system → product → product_category → eu_ai_act_subcategory`). **DM-S4b (INV-82, D-71):** when `use_case.product_category_id` is set, `resolve_classification` governs from that category's primary mapping (declared path); `ClassificationDisposition` (`AUTHORITATIVE`/`DOWN_SELECTION`) is computed by comparing the declared tier to the product-wide-highest tier. Unresolvable (no product / no primary mapping, or declared category has no primary mapping) → `REQUIRES_CONTEXT` → context gate. `off_label=True` on the snapshot when product present but no declared category (D-72). Snapshots versioned; overrides tracked (proposed/override tier, justification, actor) in `Classification` + `AuditEvent`. Resolution/persistence separated (INV-11; PAT-5).
...
**Operational fix (2026-06-27) — BFF session infinite-redirect loop (`D-62`):** Two related bugs caused an infinite login loop whenever the dev server restarted. (1) The in-memory session store is wiped on restart; the browser retains the old cookie; every proxy call returned 401; `createQueryClient`'s D-50 handler redirected to login; a fresh login set a new session cookie in the callback response, but the browser did not reliably overwrite the stale cookie on the redirect response under rapid redirect chaining — causing each fresh login to still produce a 401 on the next proxy call. Fix: the proxy's 401 response (when `getSession()` returns null) now deletes the session cookie, so the stale UUID is cleared from the browser before the re-login begins. (2) The proactive refresh path added in UI-V2-DEPTH-LAYOUT called `sessionStore.destroy()` in its catch block and returned null on any Cognito hiccup, re-entering the same loop. Fix: on refresh failure return the existing session unchanged — the API rejects a genuinely expired token with 401, and D-50 converts that to a single clean re-login. Files changed: `apps/tenant/app/api/proxy/[...path]/route.ts`, `apps/tenant/lib/auth/get-session.ts`.

### Intake / registration wizard (`UI-F1-INTAKE`)
`apps/tenant/app/systems/new` — the first tenant feature surface (adoption face, `UX.md` §5), pure wire-up over landed S1/S2/S5 routes plus the F0 shared packages, with one additive backend addition (`DF1-9`: the six intake-vocab list routes). A single-page wizard (`page.tsx`) driven by a reducer (`wizard-state.ts`).

**DM-S2 (DF-D2-1, DF-D2-3, closes DF-D1-2):** The wizard's capture spine was refactored. Steps: drill-down (category → vendor → product, or "not in catalogue / in-house" exit) → structured capture (system-stable facts only: name, operator role, hosting model, lifecycle stage, purpose — no network call; dispatches `INTAKE_DONE`) → catalogue-fact prefill confirm (display-only, `DF1-4`/`DF1-8`, re-keyed to `catalogueProductId` via `GET /v1/catalogue/products/{id}/prefill`, enabled only when product is non-null) → use-case create (`POST /v1/registrations` — the atomic endpoint, dispatches `REGISTERED`), branching on the create response in this precedence order: `requires_context` → context-question gate sub-surface (preview-before-commit, the UNRESOLVED loop, `POST .../context`) → resolved tier + the gate-1 override ladder (`POST .../classify/override`, structurally tier/subcategory-matched) → whose-court (`GET .../lifecycle`, reading `blocking.responsible_party`/`reason`).

Four use-distinguishing context controls (`usage_context_id`, `human_oversight_type_id`, `data_category_ids`, `affected_party_ids`) moved from the intake-capture step to the use-case-create step (DF-D1-2 closed). `IntakeCaptureStep.onSubmit` now passes `IntakeCaptureFacts` (no network call); `UseCaseCreateStep` now holds the four context controls and fires `useRegister` (replaced `useCreateSystem` + `useCreateUseCase`). `SystemCreate` contract removed from `@irontrust/api-client`. `usePrefill` re-keyed from `systemId` to `catalogueProductId`.

Role-aware at the page root via `GET /v1/me` (`D-24`): system_owner gets the full spine; a bare contributor/member gets a "needs a system owner" empty-state; reviewer/authoriser/auditor get an assurance-read message. `override_tier`/`classify/override` controls are `SodAction`-barred (absent, not disabled) for non-system_owner. Confirmed absent: `POST .../advance` and `GET /systems/{id}/approval-status`. Reviewer sign-off (`DF1-3`) and `If-Match`/`FE-6` (`DF1-5`, asserted dormant) are out of scope for F1.

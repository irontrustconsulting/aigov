...
`POST /v1/registrations` (DM-S2, D-65) — single atomic endpoint, single transaction: creates `System` (deployment-stable facts: `name`, `is_custom`, `catalogue_product_id`, `operator_role_id`, `hosting_model_id`, `lifecycle_stage`, `owner_user_id`, `purpose`) + first `UseCase` (use-distinguishing context: `title`, `use_case_purpose`, `usage_context_id`, `human_oversight_type_id`, `data_category_ids`, `affected_party_ids`) + snapshots `Classification` — all via `db.flush()` within the same request transaction (INV-27). Returns `RegistrationRead { system: SystemDetail, use_case: UseCaseRead, classification: ClassificationRead }`. Gate: `system_owner`. INV-78: the only route that constructs a `System`. Rollback on any mid-handler exception leaves no orphan rows (verified by `TestRegistrationAtomicity`). `db.flush()` is used inside the handler; caller's `get_tenant_db` commits at request end.

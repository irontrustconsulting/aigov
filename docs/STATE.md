...
**Current through:** DM-S1 (context relocation — `usage_context_id`, `human_oversight_type_id`, `data_category_ids`, `affected_party_ids` moved from `system`/`system_data_category`/`system_affected_party` to `use_case`/`use_case_data_category`/`use_case_affected_party`; DPIA/FRIA feeder pre-fill and DPIA applicability now per-use-case; D-63, D-64, INV-76, INV-77, DF-D1-1..4 minted). Prior: UI-V2-DEPTH-LAYOUT (elevation + surface-layering tokens FE-26; `SectionGroup` + `PageScaffold` width prop FE-27; `VerdictChip` label map FE-16 ALTER; dashboard zero-systems scaffolded-empty D-61; INV-74, INV-75, DF-V2-1..4 minted; `UI-V2-OPERATOR-SKIN` formally retired — absorbed into C-track). Sprints 1–7b + UI-F1..F8 + UI-F9 + UI-V0 + UI-V1 + UI-C0 + UI-C1 + UI-C2 + UI-V2 + DM-S1 built. Per-surface composition passes (F1–F8) tracked in the composition-debt register below.
...
`POST /v1/systems` captures deployment-stable intake metadata: single-select vocab FKs (`operator_role_id`, `hosting_model_id`, `catalogue_product_id`), `lifecycle_stage`, `is_custom`, free-text `purpose`. Vendor derived from product. `is_custom` + any catalogue FK → 422 (DB check `ck_system_custom_no_catalogue`). Vocab IDs must be `active=true` (422); `owner_user_id` must be a tenant member. `GET /v1/systems/{id}` → `SystemDetail` (resolved labels, `use_case_count`, per-use-case states). `PATCH` updates a subset; changing `catalogue_product_id` with use cases present → 409. Gates: `POST`/`PATCH` `system_owner`; `GET` any member. Audit `system.created`/`updated`. **DM-S1 (D-63):** `usage_context_id`, `human_oversight_type_id`, `data_category_ids`/`affected_party_ids` moved from system to use-case (see Use-case registration below).

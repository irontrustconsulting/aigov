...
- **Operator RBAC** (`apps/operator/app/(console)/operators`, `UI-F8-OPERATOR-RBAC`) — two regions: (1) **operator list** — read-only `GET /platform/operators` result, columns `email`/`display_name`/`status`/`roles`/`id`; (2) **create form** — captures `email`, `display_name`, `role_key` (select defaulting `provisioner`), posts to `POST /platform/operators`, surfaces 201 "invite sent" + refetches list, 409 flags email field, 422 flags role-select. Entire surface absent (not empty-state) when `operator:create ∉ permissions` (`FE-13`, `DF7-2`). Deferred (`DF8-3`): status toggle (ACTIVE↔DISABLED), role re-grant/revoke.

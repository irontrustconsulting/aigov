...
- **`provenance_confidence` is 4-value by design.** `USER_PROVIDED` was considered and **not** added — the AIIA design review's register-fact finding resolved toward read-through over a fifth enum value. Canonicals must not claim a 5-value model: register-derived facts are handled **without** a `USER_PROVIDED` tag. Confirm the exact mechanism (read through untagged vs reusing an existing value) against `create_aiia`; PAT-8 is corrected accordingly.

...
| `provenance_confidence` | AI_SUGGESTED, CATALOGUE_CURATED, USER_CONFIRMED, USER_AMENDED | `assessment_item` (PAT-8) — **4 labels only; ORM/`pg_enum` parity restored (FIX-PROVENANCE-ENUM-DRIFT, INV-93)** |
...
- **`provenance_confidence` is 4-value by design — confirmed V-2 (UI-V0-VISUAL-FOUNDATION); ORM parity restored (FIX-PROVENANCE-ENUM-DRIFT).** `USER_PROVIDED` was considered and **not** added at the DB; the four live labels are `AI_SUGGESTED`, `CATALOGUE_CURATED`, `USER_CONFIRMED`, `USER_AMENDED`. The ORM nonetheless carried a fifth `ProvenanceConfidence.USER_PROVIDED` member as undetected debt (three `assessment_service.py` emit sites 500'd on any real insert) until FIX-PROVENANCE-ENUM-DRIFT deleted it and retagged the sites: register-fact snapshots (`_add_snapshot_item`) → `USER_CONFIRMED`; the two authoring flips (`amend_item`, `create_item_from_section`) → `USER_AMENDED`. `source_ref` (non-null only on snapshots) is now the sole discriminator distinguishing a snapshot from a worked item under the same tag (INV-94). `PAT-8` is the authoritative implementation shape; INV-93 guards ORM/`pg_enum` parity going forward. No fifth label exists in `pg_enum` and none must be added without a design-review decision.

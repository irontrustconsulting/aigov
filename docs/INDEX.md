...
- **Invariants:** `INV-1..72`, each tagged `DB` (structurally unviolable) / `CODE` (one routed locus) / `CONVENTION` (discipline only — review must catch).
- **Decisions:** `D-1..56` (+ sprint-local `DF1-3..9`, `DF3-1..7`, `DF4-1..6`, `DF5-1..11`, `DF6-1..10`, `DF7-1..3`, `DF8-1..3`, `DF-V1-1..2`, `DF-F9-1`, `DF-F9-3`, `DF-C2-1..5`); open questions `OPEN-1..4` + `OPEN-V1..V7` (`OPEN-C1` resolved at UI-C2).
- **Patterns:** `PAT-1..10`.
- **UX principles:** `UX-1..6` (`UX.md`).
- **Frontend conventions:** `FE-1..25` (`FRONTEND.md`).

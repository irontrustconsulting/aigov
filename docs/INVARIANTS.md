...
**INV-72** · CONVENTION · `DrillDownStep` single-vendor auto-skip: when `vendors.data.length === 1`, the vendor rung is skipped automatically via a `useEffect` that sets `vendorId = vendors.data[0].id`. A `vendorAutoSkipPrevented` ref must be set to `true` in `goBackFromProductRung` (when `vendors.data.length === 1`) before clearing `vendorId`, to prevent the effect from re-triggering on back-navigation. The ref must be reset to `false` in `selectSubCategory` whenever a fresh sub-category is chosen, so the auto-skip fires normally on the next sub-category with a single vendor.

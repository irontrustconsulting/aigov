...
**OPEN-4** · Genesis governance role bootstrap gap — **RECLASSIFIED, see D-78**
When a tenant is first provisioned, the owner is the only member and holds the `ADMIN` administrative role with zero governance roles. D-5 blocks self-assignment — the acting admin cannot assign governance roles to their own membership. In a single-member or newly-provisioned tenant this means no one can assign the owner a governance role via the API; the owner is stranded at the "no portfolio to show" empty state. Current dev workaround: direct DB insert into `governance_role_assignment`. Path to resolution: (a) `provision_tenant` accepts an optional initial governance role and seeds the assignment at creation time, bypassing the self-assignment gate (provisioner is the granting actor); or (b) explicit grace exception for the membership created during `provision_tenant` in `assert_governance_assignable`. **D-78 (FIX-RESUME-REGATE) reclassifies this from "blocks MVP" to deferred/out-of-MVP-scope** — single-operator tenants are supported via multiple memberships; genesis API self-assignment is deferred to exception management. Still not built.
↳ refs: D-5, INV-7, D-78; discovered: genesis bootstrap test 2026-06-24

**OPEN-5** · Resume-from-draft dead-end: derived-field re-gate (B1/INV-83) is unreachable if the parked step was `prefill` or `use-case` — **RESOLVED by FIX-RESUME-REGATE, see INV-83 ALTER, FE-36, D-77, DF-RR-1..5**
`confirmedIntakeFields`/`confirmedFactKeys`/`amendedFactKeys` are deliberately not persisted to `draft_blob` and reset to `[]` on every `RESUME_FROM_DRAFT`, "so derived fields re-gate" (B1, INV-83, CAT-4/D-74 — `wizard-state.ts` comment at the `WizardState.confirmedIntakeFields` field). But `RESUME_FROM_DRAFT` restores `step` to whatever was saved in `draft_blob` via the existing `clampStep` (`wizard-state.ts:271`), which only caps *post-boundary* steps down to `use-case` — it does not route a pre-boundary step back to `intake`. `PRE_BOUNDARY_STEPS` includes `intake`, `prefill`, and `use-case` (`wizard-state.ts:28`); if the draft was parked at `prefill` or `use-case` (i.e., past the `intake` step where the "Confirm this value" derived-field controls live — FE-32), resume drops the user straight back there with the confirm arrays empty and no visible affordance to re-satisfy them. For a catalogue-linked system whose derived defaults (`operator_role_id`, `lifecycle_stage`) are still at their seed value, `POST /v1/registrations` then unconditionally 422s (`app/routers/v1/registrations.py:170-181`) — a dead end: Continue on the use-case-create step can never succeed until the draft is discarded and restarted.
**Reproduced live (2026-07-12):** confirm both derived fields at the intake-capture step, park the draft while sitting on the use-case-create step (or later), resume, hit Continue → 422 `undispositioned_fields`.
**Resolution (FIX-RESUME-REGATE, 2026-07-12):** the resume-only step-clamp candidate floated here was rejected at design (DF-RR-1(a), DF-RR-5) — it fails without also fixing the root-cause basis bug and routes upstream instead of gating at the actual commit point. Shipped instead: (B) `SEED_INTAKE` re-derives basis value-vs-seed rather than value-presence, so a resume-restored value still at its seed correctly re-reads `"derived"`; (E-compact) a pre-commit disposition gate (FE-36) at the foot of the use-case step surfaces and lets the user resolve any still-unconfirmed derived field before Register, reachable regardless of which pre-boundary step the draft was parked at. See `INV-83` ALTER for the full mechanism and D-77 for the accompanying fact-disposition-on-resume policy (Path B).
↳ refs: D-74, INV-83, DF-D3-1, FE-32, FE-36, D-77, DF-RR-1..5, DM-S3 (draft resume); discovered: live testing of UI-UC-CREATE-COMPOSE 2026-07-12; resolved: FIX-RESUME-REGATE 2026-07-12
---
...
**D-68** · `PrefillResponse.field_prefills` carries typed `{value, basis}` pairs; `SEED_INTAKE` is the canonical seed; the submitted values ARE the seeded values
`GET /v1/catalogue/products/{id}/prefill` now returns `field_prefills: FieldPrefills | null` alongside the existing `facts` list. `FieldPrefills` is a named-key map (`hosting_model_id`, `operator_role_id`, `lifecycle_stage`, `purpose`), each value being `{value: string, basis: "catalogue" | "derived"}`. The wizard seeds intake fields on prefill-data load via `SEED_INTAKE` (empty-guarded reducer action): draft-restored values take precedence; catalogue/derived values fill empty fields; blank for custom. The user amends via the controlled intake form (`SET_INTAKE_FIELD` clears the basis to `user-set`). On Continue, the wizard state values ARE the submitted values — there is no separate "apply prefill" confirmation call. **[Partially superseded by CAT-4 / D-74, D-75]:** The clause "the server does not store which fields were seeded vs user-entered" is superseded — `POST /v1/registrations` now writes `prefill_disposition` rows (INV-83). The rejected alternative (b) "send `basis` to the server" remains rejected: `basis` is still not sent; the server *re-derives* provenance from the `confirmed_fields` signal and the seed map, not from a client-supplied basis tag.
**N4(b) forward constraint (S4b):** the `operator_role_id` derived default (`deployer`, code `"deployer"`) was the only self-deployment role at DM-S4a. Before S4b derives obligation categories from the operator role, re-confirm this is still the only applicable derived role and that no new role with a different obligation profile has been added.
Rejected: (a) apply prefill on the server at registration time — defeats the user-confirm model (§1.5 spine, D-1); (b) send `basis` to the server — the server does not record this; (c) make `PrefillStep` do the seeding — `PrefillStep` reads `facts` (display-only catalogue provenance), not `field_prefills` (structured defaults); the two stores are orthogonal.
↳ origin: DM-S4a · refs: D-1, D-69, D-70, FE-30, INV-81 · **[amended CAT-4 → D-74, D-75, INV-83]**

**D-69** · Curated prefill knowledge lives in typed `catalogue_product` columns
`catalogue_product.hosting_model_id` (FK → `hosting_model`, nullable, indexed) and `catalogue_product.intended_use` (text, nullable) are the canonical home for operator-curated product-level prefill defaults. These are GLOBAL columns (no RLS), populated via `scripts/seed/seed_catalogue_prefill.py`. The `catalogue_fact` free-form JSONB stores display-only provenance facts for the prefill-step UI (FE-5); the typed columns are the source of structured field seeding for the intake step (FE-30). The two stores are orthogonal. Uncurated products (`NULL` on both columns) degrade gracefully: the form shows blank selects and no purpose prefill; derived fields (`operator_role_id`, `lifecycle_stage`) are always present for catalogue-linked systems.
Rejected: storing curated values in `taxonomy_tags` JSONB (no FK integrity, no typed queries); a separate `catalogue_product_prefill` table (premature normalisation for two sparse nullable columns).
↳ origin: DM-S4a · refs: D-68, INV-81, alembic `cce02cda26a4`

**D-70** · Derived structured-field defaults apply only to catalogue-linked systems; custom systems receive no prefill
...
**D-74** · Intake prefill disposition model: derived fields require explicit confirm; catalogue-seeded fields take submit-as-confirmation
Two distinct disposition tracks for seeded intake fields at `POST /v1/registrations` (INV-83): (1) **Derived defaults** (`operator_role_id`, `lifecycle_stage`; `basis="derived"` from `_build_field_prefills`) — the user has not chosen these values, so confirming at the seed value is ambiguous without an explicit signal. The UI presents a `DerivedConfirmControl` per field; the Continue button is disabled until all derived-seeded fields are either confirmed (dispatches `CONFIRM_INTAKE_FIELD`) or edited (value change clears confirm, becomes `USER_AMENDED`). (2) **Catalogue-seeded fields** (`hosting_model_id`, `purpose`, `name`; `basis="catalogue"`) — the user has actively reviewed and submitted, so submission of the seed value is treated as confirmation (`USER_CONFIRMED`). Any deviation is `USER_AMENDED`.
**Why separate tracks:** derived defaults are system-inferred and may be wrong (e.g., the product is deployed in-house, not by a classic deployer); catalogue-seeded values are curated operator knowledge that the user has already read and chosen to keep.
Rejected: (a) require explicit confirm for all seeded fields — overloads the intake step with confirmations for fields the user has clearly reviewed; (b) require no confirm for derived fields — leaves ambiguous whether the user actually reviewed the default.
↳ origin: CAT-4 · refs: INV-83, D-68, D-75, FE-32

**D-75** · Prefill provenance stored in `prefill_disposition` table; written at registration and re-upserted on PATCH
Every seeded field's disposition is persisted as a `prefill_disposition` row keyed `(system_id, field_key)` with a `UNIQUE` constraint. The row is written at `POST /v1/registrations` (initial disposition) and upserted on `PATCH /v1/systems/{id}` when a provenance-bearing field changes (re-disposition to `USER_AMENDED`). `field_key` uses the API snake_case name for intake fields (`operator_role_id`, `lifecycle_stage`, etc.) and `"fact:<key>"` / `"fact_amended:<key>"` for catalogue fact dispositions (namespace separation — no clash). `SystemDetail` carries `field_provenance: dict[str, str] | None` (a projection from the `prefill_disposition` rows) to expose per-field provenance to the UI without requiring a separate read.
Why a dedicated table rather than columns on `system`: (a) the field set is open (any intake field or catalogue fact); (b) each disposition needs an `actor_user_id` and a `created_at` — a JSONB blob on `system` loses that; (c) the UNIQUE constraint is cleaner on a dedicated table than on a composite column.
...
## FIX-RESUME-REGATE decisions (OPEN-5)

**D-77** · Path B: on resume, an unreviewed catalogue fact retains catalogue-curated provenance; the client never synthesises `USER_CONFIRMED` for it
`RESUME_FROM_DRAFT` resets `confirmedFactKeys`/`amendedFactKeys` to `[]` (B1, INV-83), so a draft resumed past `prefill` carries no fact dispositions in `confirmed_fields`, and `_dispose_fact_prefill` writes no rows for it (F3, no 422 — a silent gap, not a hard failure). Fix: the client does not attempt to close this gap by synthesising confirmation. A commit-time "everything not explicitly amended counts as confirmed" fallback was considered and dropped, because `amendedFactKeys` is itself wiped on resume — such a fallback would resend a pre-park-amended fact as `fact:<key>` (USER_CONFIRMED) instead of `fact_amended:<key>` (USER_AMENDED), fabricating a `system.field_confirmed` audit event (D-6) for a fact the user had actually overridden. Instead: send nothing for unreviewed facts, same as today; the pre-commit disposition gate (FE-36) surfaces this as a visible, truthful, non-blocking recap — "N catalogue facts have not been reviewed this session. They will be kept as recorded in the catalogue unless you review them." — with a link back to `prefill` for genuine review. This is a durable disposition-on-resume policy, not a sprint-local call.
Rejected: (a) persist `amendedFactKeys` in `draft_blob` and restore it on resume (Path A) — correct in principle, but a `draft_blob` shape change and a split of the INV-83 B1 transience rule (amendments durable, confirms transient); heavier than the gap warrants for a draft-time bookkeeping concern. (b) server-side default-confirm of any fact absent from `confirmed_fields` — fabricates provenance from the server side instead of the client, same defect. (c) commit-time synthesise-all-confirmed (`confirmedFactKeys = productFactKeys \ amendedFactKeys`) — the original candidate; drops the B2 defect above precisely because `amendedFactKeys` is empty on resume.
↳ origin: FIX-RESUME-REGATE · refs: INV-83, D-74, D-75, FE-32, FE-36, D-6, DF-RR-3

**D-78** · OPEN-4 (genesis governance role bootstrap gap) reclassified from "blocks MVP" to deferred/out-of-MVP-scope
Single-operator tenants are supported today via multiple memberships (the sole operator holds more than one governance role, or a second membership is created for the granting actor) — the genesis-bootstrap gap OPEN-4 describes (a brand-new single-member tenant where the owner cannot self-assign a governance role, D-5) is a real gap but not a blocker for shipping: it affects only the first-minute state of a newly-provisioned tenant before any second member or role-assignment action, and existing exception-management/support-workaround paths (direct DB insert today; a proper exception-management flow is the intended eventual fix) cover it. Reclassified from "blocks real single-person onboarding" to a deferred item. Not built; OPEN-4 stays open, now correctly scoped.
Rejected: build the genesis-bootstrap fix now (either `provision_tenant`-seeded initial role or a grace exception in `assert_governance_assignable`) as part of this sprint — unrelated to OPEN-5/FIX-RESUME-REGATE, and the reclassification alone unblocks nothing that was actually blocking MVP shipment.
↳ origin: FIX-RESUME-REGATE (carry-over, unrelated to OPEN-5) · refs: OPEN-4, D-5, INV-7

**DF-RR-1** · Approach: B + E-compact (sprint-local)
Fix F2 (basis mislabel) at root — re-derive basis from value-vs-seed (B) — and surface the confirm affordance at the commit point — a pre-commit disposition gate, shown only when disposition is incomplete (E-compact). The invariant is "every seeded field dispositioned before commit"; E places the gate at the commit rather than relying solely on an earlier step being reachable.
Rejected: (a) resume-to-intake clamp alone (the OPEN-5 candidate) — fails without B (the client would still silently mislabel a kept derived default as `"user-set"` and render no confirm control even after being routed back to `intake`); routes upstream instead of gating at commit. (b) B + a full editable fact-card surface at the boundary (E-full) — duplicates the `prefill` surface for a bolted-on gate. (c) persist disposition in `draft_blob` — contradicts the INV-83 B1 transience rule. (d) an editable review-and-submit summary as the terminal disposition surface — cleaner than a bolted-on gate in isolation, but a third wizard shape (neither the current linear wizard nor the intended context-driven one); inline disposition at the commit point is the chosen long-run direction (an intelligent wizard with conditional field presentation, Appendix A forward item) and generalises into a per-screen gate, so E-compact is not throwaway the way an end-summary would be.
↳ origin: FIX-RESUME-REGATE (sprint-local DF) · refs: FE-36, INV-83, D-77

**DF-RR-2** · Fact handling: E-compact, non-blocking (sprint-local)
The pre-commit gate shows a facts recap, not editable cards; amendment stays single-home at `prefill` via a Review-facts link. Facts never hard-gate Register — only the two intake-derived fields (`operator_role_id`, `lifecycle_stage`) do.
Rejected: E-full editable fact cards at the gate (DF-RR-1(b)) — duplicates `prefill`.
↳ origin: FIX-RESUME-REGATE (sprint-local DF) · refs: FE-36, D-77

**DF-RR-3** · B2 = Path B (honest absence, no synthesised confirmation) (sprint-local)
The commit-time synthesise-all candidate (`confirmedFactKeys = productFactKeys \ amendedFactKeys`) is dropped in favour of Path B — see D-77 for the durable policy and full rationale. This entry records the sprint-local disposition of the drafting-stage candidate itself.
Rejected: commit-time synthesise-all as originally drafted (fabricates provenance on resume, the B2 defect D-77 describes).
↳ origin: FIX-RESUME-REGATE (sprint-local DF) · refs: D-77

**DF-RR-4** · INV-83 re-gate kept, not relaxed (sprint-local)
The "every seeded field dispositioned before commit" invariant is enforced end-to-end via B (basis re-derive) plus E (the pre-commit gate) — not relaxed or bypassed for the resume path.
Rejected: relaxing INV-83's re-gate requirement for resumed drafts (DF-RR-1(c)'s draft_blob-persistence alternative would have amounted to this).
↳ origin: FIX-RESUME-REGATE (sprint-local DF) · refs: INV-83

**DF-RR-5** · No resume-step clamp (sprint-local)
`RESUME_FROM_DRAFT` keeps restoring the saved step as-is; resume lands wherever the user left off. The pre-commit gate (E) makes a step clamp redundant — disposition is satisfiable at the commit point regardless of which pre-boundary step was parked.
Rejected: a resume-to-intake or resume-to-earlier-step clamp (the original OPEN-5 candidate fix) — superseded by E-compact; see DF-RR-1(a).
↳ origin: FIX-RESUME-REGATE (sprint-local DF) · refs: FE-36, OPEN-5
...
**D-82** · Hold `provenance_confidence` at four labels; delete the phantom `ProvenanceConfidence.USER_PROVIDED` ORM member; retag its three emit sites; correct `_is_pristine`
The live `pg_enum` has always been 4-value (`AI_SUGGESTED`, `CATALOGUE_CURATED`, `USER_CONFIRMED`, `USER_AMENDED` — DATA-MODEL §5, D-1, V-2), but the ORM enum still carried a fifth `USER_PROVIDED` member, designed in the original AIIA sprint for user-origin content with no system default and never cleaned up when the model consolidated to the four-value confirm/amend ladder. Three `assessment_service.py` sites (`_add_snapshot_item`, `amend_item`, `create_item_from_section`) still emitted it, so any insert on those paths raised `InvalidTextRepresentation` — live on the snapshot path (the reported defect), latent on the two authoring paths. Resolution: (1) hold the DB at four labels — `ALTER TYPE ADD VALUE` was rejected as reversing a locked decision, needing a migration, and re-widening the vocabulary for a distinction with no read-time governance weight; (2) `_add_snapshot_item` → `USER_CONFIRMED` (a register snapshot is a fact the user stood behind at registration, INV-83, passively inherited rather than authored — founder-confirmed); (3) the two authoring flips (`amend_item`'s `CATALOGUE_CURATED →` transition, `create_item_from_section`'s create-with-response) → `USER_AMENDED` (active authoring of a blank curated prompt, outside the override-rate metric population per PRD §7.1, so a defect-class retag rather than a metric change — `USER_CONFIRMED` was rejected as reading like passive acceptance of a default that never existed); (4) `_is_pristine`'s worked-detection corrected from a tag-keyed `or_(IN(...), and_(== USER_PROVIDED, source_ref IS NULL))` to a single `provenance IN (USER_CONFIRMED, USER_AMENDED) AND source_ref IS NULL` predicate — post-retag, `source_ref` is the only thing left distinguishing a snapshot from a worked item under the same `USER_CONFIRMED` tag. Zero-row guarantee: no real row could ever carry `USER_PROVIDED` (every insert 500'd), so the member deletion needed no data migration. Supersedes the V-5 pre-flight note in `assessment-page-client.tsx` ("ProvenanceConfidence is 5-value") — the fifth member was debt, not intent.
Rejected: `ALTER TYPE ADD VALUE 'user_provided'` (above); per-field inherit from `prefill_disposition` for snapshot provenance (a second home for a single-homed truth, and partial — some snapshot fields have no disposition row).
↳ origin: FIX-PROVENANCE-ENUM-DRIFT · refs: PAT-8, PAT-9, INV-13, INV-14, INV-17, INV-23, INV-36, INV-83, INV-93, INV-94, D-1, D-21, FE-15, DATA-MODEL §2/§5, V-2, V-5

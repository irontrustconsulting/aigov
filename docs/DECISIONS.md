...
**OPEN-5** · Resume-from-draft dead-end: derived-field re-gate (B1/INV-83) is unreachable if the parked step was `prefill` or `use-case`
...
**Candidate fix (not yet applied — flagged for design review before implementation):** a resume-only step clamp, separate from the existing `clampStep` (which is also used for persist-on-transition and must keep its current post-boundary-only capping behaviour — reusing it directly for this would wrongly force every mid-flow draft save back to `intake`). The resume-only variant would route a restored `prefill`/`use-case` step back to `intake` specifically inside `RESUME_FROM_DRAFT`, so the confirm gates are re-satisfied in sequence (`intake` → `prefill` → `use-case`) before the user can reach Continue again. Other shapes (e.g., an inline error banner surfacing the 422's `undispositioned_fields` directly on the use-case-create step, or a targeted "some fields need reconfirming" resume notice) were not evaluated — this needs a design pass, not just the mechanical fix, before it ships.
↳ refs: D-74, INV-83, DF-D3-1, FE-32, DM-S3 (draft resume); discovered: live testing of UI-UC-CREATE-COMPOSE 2026-07-12
---

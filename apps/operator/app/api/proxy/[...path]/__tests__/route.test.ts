...
describe("BFF proxy route", () => {
  test("forwards the request with the bearer ID token attached", async () => {
    ...
    const request = new NextRequest("http://localhost:3001/api/proxy/v1/me", {
  ...
  test("forwards If-Match on a mutation", async () => {
    ...
    const request = new NextRequest("http://localhost:3001/api/proxy/v1/use-cases/1", {
      method: "PATCH",
  ...
  test("403s a cross-origin mutation before checking the session", async () => {
    ...
    const request = new NextRequest("http://localhost:3001/api/proxy/v1/systems", {
      method: "POST",
